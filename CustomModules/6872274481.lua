...
runcode(function()
			...
			if tppos2 then 	
				task.spawn(function()
					...
					if root and tppos2 then 
						...
						RunLoops:BindToHeartbeat("DeathTPV2", 1, function(dt)
							if root and tppos2 then 
						...
						createwarning("TpDirection Remake", "Teleported.", 5)
						tppos2 = nil
					end
				end)
			end
			...
			Function = function(callback)
				if callback then
					...
					if ray then 
						...
						local warning = createwarning("TpDirection Remake", "Reset In 1.5 Sec", 1.5)
								  wait(1.5) 
								  game:GetService("ReplicatedStorage").rbxts_include.node_modules:FindFirstChild("@rbxts").net.out._NetManaged.ResetCharacter:FireServer()
	...
	runcode(function()
		...
		VeloHighJump = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
			["Name"] = "HighJump",
			["HoverText"] = "By Maxlaer :)",
			["Function"] = function(callback)
				if callback then
					workspace.Gravity = 196.2
					if entity.isAlive then
						entity.character.HumanoidRootPart.Velocity = entity.character.HumanoidRootPart.Velocity + Vector3.new(0, 300, 0)
					end
